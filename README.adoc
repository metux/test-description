...
+-env			  (environment settings)
	...
	overlay.list	  (list of overlays, that should be used for compile
			   tests)
	arm.properties	  ((cross) compile settings for architecture, if needed)
	boottest.list	  (list of all boottests)
	email.list	  (list of notification email addresses)

+-compile		  (configurations and overlays for compile test)
	...
	+-overlays	  (folder with all defined overlays)
